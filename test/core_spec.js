...
import {setBoard, updateSquare, resetCharacter, setLevels, pickupWeapon, setPosition} from '../src/core';
...
describe('game logic', () => {
  ...
  describe('pickupWeapon', () => {
    it('changes the characters weapon', () => {
      const characterState = fromJS({ level: 1, xp: 80, health: 70, weapon: 0 });
      const nextState = pickupWeapon(characterState, 6);
      expect(nextState).to.equal(fromJS({ level: 1, xp: 80, health: 70, weapon: 6 }));
      expect(characterState).to.equal(Map({ level: 1, xp: 80, health: 70, weapon: 0 }));
    });
  });

  describe('setPosition', () => {
    it('changes the characters coordinates', () => {
      const characterState = fromJS({ level: 1, position: [2, 2], xp: 80, health: 70, weapon: 0 });
      const nextState = setPosition(characterState, [2, 3]);
      expect(nextState).to.equal(fromJS({ level: 1, position: [2, 3], xp: 80, health: 70, weapon: 0 }));
      expect(characterState).to.equal(fromJS({ level: 1, position: [2, 2], xp: 80, health: 70, weapon: 0 }));
    });
  });
});
